...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT id, name, participant, price FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( id, name, participant, price ) VALUES ( ?, ?, ?, ? )";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET id = ?, name = ?, participant = ?, price = ? WHERE id = ?";
	...
	/** 
	 * Index of column participant
	 */
	protected static final int COLUMN_PARTICIPANT = 3;
	...
	public ItemsPk insert(Items dto) throws ItemsDaoException
	{
		...
		try {
			...
			if (dto.isParticipantModified()) {
				if (modifiedCount>0) {
					sql.append( ", " );
					values.append( ", " );
				}
		
				sql.append( "participant" );
				values.append( "?" );
				modifiedCount++;
			}
		
			if (dto.isPriceModified()) {
				if (modifiedCount>0) {
					sql.append( ", " );
					values.append( ", " );
				}
			...
			if (dto.isParticipantModified()) {
				stmt.setString( index++, dto.getParticipant() );
			}
		
			if (dto.isPriceModified()) {
				stmt.setLong( index++, dto.getPrice() );
			}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE participant = ? ORDER BY participant", new Object[] { participant } );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE price = ? ORDER BY price", new Object[] {  new Long(price) } );
	}
	...
	protected void populateDto(Items dto, ResultSet rs) throws SQLException
	{
		...
		dto.setParticipant( rs.getString( COLUMN_PARTICIPANT ) );
		dto.setPrice( rs.getLong( COLUMN_PRICE ) );
		reset(dto);
	}
	...
	protected void reset(Items dto)
	{
		...
		dto.setParticipantModified( false );
		dto.setPriceModified( false );
	}
