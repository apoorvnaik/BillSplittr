...
public class NewsFindByPrimaryKeyAction extends Action
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			...
			// execute the finder
			News dto = dao.findByPrimaryKey(_id);
			...
			newsForm.setCrudMethod( request.getParameter("crudMethod") );
			newsForm.setId(String.valueOf( dto.getId() ));
			newsForm.setContent(dto.getContent());
			newsForm.setDated(dto.getDated() == null ? null : DateFormat.getDateTimeInstance().format( dto.getDated()));
			// store the results
			request.getSession().setAttribute( "News", newsForm );
